...
def work_key(title, author=""):
    """The Open Library work that best matches, as "/works/OL…W", or "".

    The author narrows it a great deal — every catalogue has a dozen books called Dark Matter —
    but a name spelled differently there finds nothing at all, so a search that comes back empty
    is tried again on the title alone.
    """
    for query in ([("title", title), ("author", author)] if author else []), [("title", title)]:
        if not query:
            continue
        url = f"{API}/search.json?" + urllib.parse.urlencode(dict(query) | {"limit": "1",
                                                                            "fields": "key"})
        docs = (_get(url) or {}).get("docs") or []
        if docs and docs[0].get("key", "").startswith("/works/"):
            return docs[0]["key"]
    return ""
